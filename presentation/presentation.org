...
Object parser is similar, except we use open/close curly braces, and key-value
pairs separated by commas. A key-value pair is itself a string followed by a
colon followed by a value.

For the object and array parsers, each time we recurse through ~value_parser~
again, we decrement the ~Depth~ template value so that we are producing the next
...
\\
\\
It's a good starting point, but there are a few more problems to consider.
